...
def main():
    ...
    for epoch in range(1,n_epoch+1):
        ...
        loss_seq = 0
        model.reset_state()
        #tmax = random.randint(0,4)
        #tmax = random.uniform(0,5)
        #tnum = int(round(tmax*15))


        t = pi*np.linspace(0,tmax,tnum)
        x_t,y_t = cycloid(t)
        y_t = y_t + make_noise(y_t.shape[0])
        ...
        for i in range(1,tnum+1):
            ...
            if((i == tnum)):
                ...
                x = np.empty((0,1))
                y = np.empty((0,1))
        print ('#epoch:',epoch,"#Number:",tmax,'#train:',loss_seq,'     #time',time.time()-start)
        ...
        if(epoch%100 == 0):
            tmax_test = 4
            tnum_test = tmax_test*15

            t_test = pi*np.linspace(0,tmax_test,tnum_test)
            x_t_test,y_t_test = cycloid(t_test)
            #y_t_add_noise = y_t + make_noise(y_t.shape[0])
            y_prd_test = np.empty((0,1))
            x_test = np.empty((0,1))
            for i in range(1,tnum_test+1):
                x_test = np.append(x_test,x_t_test[i-1])
                if((i == tnum_test)):
                    x_test = np.reshape(x_test.astype(np.float32),[-1,1])
                    model.reset_state()
                    prd_test = model.predict(chainer.Variable(x_test),train=False)
                    #print(prd.shape)
                    y_prd_test = np.append(y_prd_test,prd_test)
                    x_test = np.empty((0,1))

            plt.plot(x_t_test,y_prd_test,'--r')
            plt.plot(x_t_test,y_t_test,'b')
            plt.xlim(0,pi*tmax_test*a)
            plt.ylim(0,pi*tmax_test*a/2)
